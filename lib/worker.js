(function() {
  var Logger, base64Image, close_response, config, fetch_url, fs, ip_allowed, logger, mime, mommy, next_instance_number, page, parse_config, run_server, strip_scripts, system, url, util, webserver;

  run_server = function() {
    var server, service;
    server = webserver.create();
    service = server.listen(config.port, function(request, response) {
      var err, escaped_fetch_url, parse_delay, parsed_fetch_url, ref, ref1, request_headers, request_url, this_inst;
  ...
  fetch_url = function(url, response, this_inst, parse_delay, request_headers) {
    ...
    page.onError = function(msg, trace) {
      ...
      return trace.forEach(function(item) {
        return logger.error(this_inst, "  ", item.file, ":", item.line);
      });
    };
